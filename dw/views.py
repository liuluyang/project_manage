...
@login_required
@views_permission
def newshot(request,*args,**kwargs):    #新建镜头任务单
    project_archive = kwargs['project']  #项目归档判断
        ...
        if summary is not None and len(summary)>0:
            ...
            if image == 'tiff':
                ...
                create_task = Taskorder.objects.create(summary=summary, creator=creator, description=description,
                                                       type=type, milestone=milestone, reporter=user_now,
                                                       component=component, priority=priority, task_image=task_image,
                                                       owner=owner, version=version, display_id=display_id,
                                                       status=statuss[0], owner_project=owner_project,
                                                       start_date=task_start_date, duration=duration,
                                                       end_date=task_end_date,grade=grade
                                                       , predict_start_date=task_start_date, predict_duration=duration,
                                                       predict_end_date=task_end_date)
            ...
            errors.append('请输入镜头号')
    return render_to_response('create_shot.html',locals())
...
@login_required
@views_permission
def newassets(request,*args,**kwargs):    #新建资产任务单
    project_archive = kwargs['project']  #项目归档判断
    ...
    if request.method == 'POST':
        ...
        if summary is not None and len(summary)>0:
            ...
            errors.append('请输入资产名')
    return render_to_response('create_assets.html',locals())
